...
namespace FellowOakDicom
{
    ...
    public class DicomDictionary : IEnumerable<DicomDictionaryEntry>
    {
        ...
        private static readonly String DictionaryPath = Path.Combine(Application.streamingAssetsPath, "FO-DICOM", "Dictionaries");
            ...
            lock (_lock)
            {
                if (_default == null)
                {
                    ...
                    try
                    {
                        ...
                        var path = Path.Combine(DictionaryPath, "DICOMDictionary.xml.gz");
                    ...
                    if (loadPrivateDictionary.GetValueOrDefault(true))
                    {
                        try
                        {
                            ...
                            var path = Path.Combine(DictionaryPath, "PrivateDictionary.xml.gz");
                            using var stream = File.OpenRead(path);
                            var gzip = new GZipStream(stream, CompressionMode.Decompress);
                            var reader = new DicomDictionaryReader(dict, DicomDictionaryFormat.XML, gzip);
                            reader.Process();
